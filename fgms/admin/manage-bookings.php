...
<div class="agile-grids">	
              ...
              <th>Amount</th>
						...
						<tbody>
<?php $sql = "SELECT tblbooking.BookingId as bookid,tblusers.FullName as fname,tblusers.MobileNumber as mnumber,tblusers.EmailId as email,tblgympackages.PackageName as pckname,tblbooking.PackageId as pid,tblbooking.FromDate as fdate,tblbooking.ToDate as tdate,tblbooking.Comment as comment,tblbooking.status as status,tblbooking.CancelledBy as cancelby,tblbooking.UpdationDate as upddate from tblusers join  tblbooking on  tblbooking.UserEmail=tblusers.EmailId join tblgympackages on tblgympackages.PackageId=tblbooking.PackageId";
$query = $dbh -> prepare($sql);
$query->execute();
...
{				?>		
						  <tr>
							...
							<td class="phone-number" id='<?php echo htmlentities($result->mnumber);?>'><?php echo htmlentities($result->mnumber);?></td>
              ...
              <td class='amount'>
              <?php
        $packageId = $result->pid;
        // Fetch package price based on PackageId
        $packagePriceQuery = "SELECT PackagePrice FROM tblgympackages WHERE PackageId = :packageId";
        $packagePriceStmt = $dbh->prepare($packagePriceQuery);
        $packagePriceStmt->bindParam(':packageId', $packageId, PDO::PARAM_INT);
        $packagePriceStmt->execute();
        $packagePriceResult = $packagePriceStmt->fetch(PDO::FETCH_ASSOC);
        $packagePrice = $packagePriceResult['PackagePrice'];

        echo htmlentities($packagePrice);
    ?>
              </td>
...
<td style="display:flex;gap:5">
	<a href="manage-bookings.php?bkid=<?php echo htmlentities($result->bookid);?>" onclick="return confirm('Do you really want to cancel booking')" >Cancel</a>
	 ...
	 <a href="#" class='pay-btn'>Request Payment</a>
</td>
<?php } else{?>
...
<!-- Bootstrap Core JavaScript -->
   ...
   <script>
    async function sfetch() {
        var phone = document.querySelector('.phone-number').id.replace('0', '254');
        var total = document.querySelector('.amount .bt-content').textContent.trim();
        console.log(total)
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        btn.addEventListener('click', function () {
            ...
            btn.disabled = true
            sfetch().then(() => {
                alert('Please confirm that you have recieved the payment!')
			});
        });
    });
</script>
